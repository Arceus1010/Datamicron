...
const EZDATA_VIDEO_ID = 'HhpJz4CLafI'
...
const WHY_EZDATA = [
  {
    ...
    title: 'Automated Data Quality Assurance',
    description:
      'Advanced AI algorithms autonomously detect and correct inconsistencies, errors and missing values — eliminating the need for manual data cleansing.',
  },
  {
    id: 'prep',
    icon: (
    ...
    description:
      'Data preparation simplified for scientists, analysts and business professionals — so teams focus on deriving insight rather than managing cleansing tasks.',
  },
  {
    id: 'tools',
    icon: (
    ...
    title: 'User-Friendly Web-Based Tools',
    description:
      'An innovative set of web-based tools for importing and wrangling complex datasets, with efficient organisation through a simple point-and-click interface.',
  },
]

const CAPABILITIES = [
  { title: 'Dynamic Schema Mapping', description: 'Adapts to varied data structures for seamless integration with multiple sources.' },
  { title: 'Automatic Schema Detection', description: 'Data structures configured without manual input, streamlining every integration.' },
  { title: 'Customizable Cleansing Rules', description: 'Define custom rules so data quality is tailored to specific business needs.' },
  { title: 'ML & Anomaly Detection', description: 'Machine learning flags anomalies, improving accuracy and enabling predictive analytics.' },
  { title: 'Audit Trail & Versioning', description: 'A complete trail of every data change, with versioning to restore previous states.' },
  { title: 'Data Ingestion', description: 'Bring datasets in from across the business and transform them in one place.' },
  { title: 'Point-and-Click Wrangling', description: 'Organise and reshape complex datasets without writing transformation code.' },
  { title: 'Cleansing Knowledge Base', description: 'Rules and corrections learned on one dataset carry forward to the next.' },
]
...
// Cleansing visual — messy rows resolving into clean ones
const RAW_ROWS = ['null', '12/13/2024', 'ACME  Corp.', '—', 'acme corp']

const CLEAN_ROWS = ['0.00', '2024-12-13', 'ACME Corp', '148.20', 'ACME Corp']

function CleansingVisual() {
  return (
    <div className="relative w-full max-w-lg">
      ...
      >
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold uppercase tracking-widest text-white/40">Cleansing run</span>
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-center">
          ...
          <div className="flex flex-col gap-2">
            <span className="text-[10px] uppercase tracking-wider text-white/30 mb-0.5">Raw</span>
            {RAW_ROWS.map((row, i) => (
              <motion.div
                key={row + i}
                className="rounded-lg border border-red-500/20 bg-red-500/8 px-2.5 py-1.5 text-[11px] text-white/50 truncate"
          ...
          >
            ...
            <span className="text-[9px] uppercase tracking-wider text-white/30">ETL</span>
          </motion.div>

          {/* Clean column */}
          <div className="flex flex-col gap-2">
            <span className="text-[10px] uppercase tracking-wider text-white/30 mb-0.5">Cleansed</span>
            {CLEAN_ROWS.map((row, i) => (
              <motion.div
                key={row + i}
                className="rounded-lg border border-brand/30 bg-brand/12 px-2.5 py-1.5 text-[11px] text-white/70 truncate"
                initial={{ opacity: 0, x: 12 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.45, delay: 0.75 + i * 0.08, ease: EASE }}
              >
                {row}
              </motion.div>
            ))}
      ...
      >
        {['Ingest', 'Detect', 'Cleanse', 'Transform'].map((item) => (
...
export default function EzData() {
  return (
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-24 w-full">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <motion.div className="flex flex-col gap-8" variants={stagger()} initial="hidden" animate="show">
              ...
              >
                EzData: <span className="text-brand">Modern ETL Tool.</span>
              </motion.h1>
              ...
              >
                A data wrangling tool that quickly cleanses and transforms your datasets — powered by AI that identifies and rectifies inconsistencies, errors and missing values on its own.
              </motion.p>
          ...
          >
            {['AI-driven cleansing', 'Automatic schema detection', 'Point-and-click wrangling', 'Full audit trail'].map((item) => (
        ...
        <div className="max-w-6xl mx-auto px-6 py-24">
          ...
          >
            ...
            <motion.h2 variants={fadeUp} transition={{ duration: 0.55, ease: EASE }} className="text-4xl lg:text-5xl font-bold text-gray-900 leading-[1.1] tracking-tight">
              Ingestion, quality, cleansing and <span className="text-brand">transformation.</span>
            </motion.h2>
            <motion.p variants={fadeUp} transition={{ duration: 0.6, ease: EASE }} className="text-xl text-gray-500 leading-relaxed">
              Powered by state-of-the-art AI algorithms, EzData autonomously identifies and rectifies inconsistencies, errors and missing values within your datasets — so manual data cleaning stops being the job.
            </motion.p>
          </motion.div>

          <div className="grid md:grid-cols-3 gap-6">
            {WHY_EZDATA.map((item, i) => (
              <motion.div
                key={item.id}
      ...
      <section className="bg-gray-50">
        <div className="max-w-6xl mx-auto px-6 py-24">
          ...
          >
            ...
            <motion.h2 variants={fadeUp} transition={{ duration: 0.55, ease: EASE }} className="text-4xl lg:text-5xl font-bold text-gray-900 leading-[1.1] tracking-tight">
              Data cleansing with <span className="text-brand">machine learning.</span>
            </motion.h2>
            <motion.p variants={fadeUp} transition={{ duration: 0.6, ease: EASE }} className="text-xl text-gray-500 leading-relaxed">
              Schema handled automatically, rules tailored to your business, anomalies surfaced by machine learning — and every change tracked end to end.
            </motion.p>
          </motion.div>
        ...
        <div className="max-w-5xl mx-auto px-6 py-24">
          ...
          >
            ...
            <motion.h2 variants={fadeUp} transition={{ duration: 0.55, ease: EASE }} className="text-4xl lg:text-5xl font-bold text-white leading-[1.1] tracking-tight">
              See the tool <span className="text-brand">at work.</span>
            </motion.h2>
            <motion.p variants={fadeUp} transition={{ duration: 0.6, ease: EASE }} className="text-xl text-white/55 leading-relaxed">
              Watch a messy dataset move through EzData — imported, profiled, cleansed by machine learning and transformed into something ready to analyse.
            </motion.p>
          </motion.div>
          ...
          >
            <iframe
              ...
              src={`https://www.youtube-nocookie.com/embed/${EZDATA_VIDEO_ID}`}
              title="EzData product walkthrough"
