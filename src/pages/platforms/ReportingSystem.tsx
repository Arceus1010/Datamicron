...
const CAPABILITIES = [
  { title: 'Intuitive Report Building', description: 'A drag-and-drop builder with no coding expertise required — design tailored reports in less time.' },
  { title: 'Time-Saving Automation', description: 'Schedule routine reports to run on their own, cutting manual effort and improving efficiency.' },
  { title: 'Audit Trail', description: 'Track every change with confidence, keeping the reporting process transparent and accountable.' },
  { title: 'Role-Based Access', description: 'Permissions ensure only authorised personnel can view and edit critical financial information.' },
  { title: 'Versatile Report Types', description: 'Operational reports, eStatement reports and list view reports from one designer.' },
  { title: 'Web-Based Delivery', description: 'Build, design and share reports from the browser — nothing to install for the people who read them.' },
]
    ...
    <div className="relative w-full max-w-lg">
      ...
      >
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold uppercase tracking-widest text-white/40">Report designer</span>
        ...
        <div className="grid grid-cols-[7.5rem_1fr] gap-3">
          ...
          <div className="rounded-xl border border-white/8 bg-brand-navy/60 p-3 flex flex-col gap-2">
            <span className="text-[10px] uppercase tracking-wider text-white/30">Fields</span>
            {FIELDS.map((field, i) => (
              <motion.div
                key={field}
          ...
          {/* Canvas */}
          <div className="rounded-xl border border-white/8 bg-brand-navy/60 p-3 flex flex-col gap-2">
            <span className="text-[10px] uppercase tracking-wider text-white/30">Canvas</span>
            {BANDS.map((band, i) => (
              <motion.div
                key={band.label}
                className={`${band.h} rounded-md border ${
                  band.label === 'Detail' ? 'border-brand/30 bg-brand/12' : 'border-white/8 bg-white/5'
                } px-2 flex items-center`}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.45, delay: 0.45 + i * 0.1, ease: EASE }}
              >
                <span className="text-[10px] text-white/35">{band.label}</span>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-24 w-full">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <motion.div className="flex flex-col gap-8" variants={stagger()} initial="hidden" animate="show">
              ...
              >
                A web-based report builder and designer that turns raw data into actionable insight — build, design and share dynamic reports with ease.
              </motion.p>
          ...
          >
            {['Drag-and-drop designer', 'Scheduled reporting', 'Role-based access', 'Built for financial institutions'].map((item) => (
        ...
        <div className="max-w-6xl mx-auto px-6 py-24">
          ...
          >
            ...
            <motion.p variants={fadeUp} transition={{ duration: 0.6, ease: EASE }} className="text-xl text-gray-500 leading-relaxed">
              Build, design and share dynamic reports with ease — on a platform shaped around how financial institutions actually report.
            </motion.p>
          </motion.div>
      ...
      <section className="bg-gray-50">
        <div className="max-w-6xl mx-auto px-6 py-24">
          ...
          >
            ...
            <motion.p variants={fadeUp} transition={{ duration: 0.6, ease: EASE }} className="text-xl text-gray-500 leading-relaxed">
              Everything from the first drag-and-drop layout to automated delivery, controlled access and a complete record of who changed what.
            </motion.p>
          </motion.div>
        ...
        <div className="max-w-5xl mx-auto px-6 py-24">
          ...
          >
            ...
            <motion.p variants={fadeUp} transition={{ duration: 0.6, ease: EASE }} className="text-xl text-white/55 leading-relaxed">
              Watch a report come together — fields dragged onto the canvas, bands laid out, and the finished output scheduled and shared.
            </motion.p>
          </motion.div>
